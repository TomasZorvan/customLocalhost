...
?>
	...
	<?php
		if ( $postinfo && ! is_page() && ( ! is_post_type_archive( 'gallery' ) && ! is_tax( 'gallery_category' ) ) ) {
			...
			et_postinfo_meta( $postinfo, et_get_option( 'harmony_date_format', 'M j, Y' ), esc_html__( '0 komentárov', 'Harmony' ), esc_html__( '1 komentár', 'Harmony' ), '% ' . esc_html__( 'komentárov', 'Harmony' ) );
			echo '</p>';
		}
...
<?php
	if ( $is_single ) :
		...
		<a href="<?php the_permalink(); ?>" class="more"><?php esc_html_e( 'Čítať ďalej', 'Harmony' ); ?></a>
<?php
	endif;
?>
	</article> <!-- end .post-->