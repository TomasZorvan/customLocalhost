			...
			<?php while ( have_posts() ) : the_post(); ?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix entry entry-content' ); ?>>
					...
					<?php
						printf( _x( 'Posted by %1$s in %2$s | %3$s', 'Meta information on single event page', 'Harmony' ),
							...
							et_get_comments_popup_link( __( '0 komentárov', 'Harmony' ), __( '1 komentár', 'Harmony' ), '% ' . __( 'komentárov', 'Harmony' ) )
						); ?>
					</p>
				...
				?>
					...
					<?php if ( $et_event_price ) : ?>
						<li><strong><?php esc_html_e( 'Cena', 'Harmony' ); ?>: </strong><?php echo esc_html( $et_event_price ); ?></li>
					<?php endif; ?>
					<?php if ( $et_event_startdate ) : ?>
						<li>
							<strong><?php esc_html_e( 'Dátum', 'Harmony' ); ?>: </strong>
						<?php
					...
					<?php endif; ?>
					<?php if ( '' != $time ) : ?>
						<li><strong><?php esc_html_e( 'Čas', 'Harmony' ); ?>: </strong><?php echo esc_html( $time ); ?></li>
					<?php endif; ?>
					<?php if ( $et_event_venue ) : ?>
						<li><strong><?php esc_html_e( 'Podujatie', 'Harmony' ); ?>: </strong><?php echo esc_html( $et_event_venue ); ?></li>
					<?php endif; ?>
					<?php if ( $et_event_location ) : ?>
						<li><strong><?php esc_html_e( 'Lokalita', 'Harmony' ); ?>: </strong><?php echo esc_html( $et_event_location ); ?></li>
					<?php endif; ?>
